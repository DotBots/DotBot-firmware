...
RTC counter, the serviced tick number, the worst tick backlog, the raw solve and
the encoder totals. It is deliberately absent from `protocol_data_type_t`: it must
not exist in a shipped target, so it claims value 13 by local convention only. Do
not register that value in the shared enum without moving this first.
...
| Coordinates above 100000 mm mean no solve | The secure side reports out-of-range values rather than a status flag. Treating them as a position puts the robot somewhere impossible. |
...
```
SEGGER_DIR="<install root>" BUILD_TARGET=dotbot-v3 BUILD_CONFIG=Debug make sandbox-dotbot-v3
```
