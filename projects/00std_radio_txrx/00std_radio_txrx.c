...
int main(void) {
    ...
    NRF_RADIO->PCNF1 = (32UL << RADIO_PCNF1_MAXLEN_Pos) |
                       (32UL << RADIO_PCNF1_STATLEN_Pos) |  // since the LENGTH field is not set, this specifies the length of the payload
    ...
    while (NRF_CLOCK->EVENTS_HFCLKSTARTED == 0) {
        ;
    }  // Wait for the clock to actually start.
    ...
    while (1) {
        if (packet[4] != 0x00) {
            ...
            if (packet[0] == 0x01) {
                NRF_P0->OUTSET = 1 << GPIO_OUTSET_PIN31_Pos;
            } else {
                NRF_P0->OUTCLR = 1 << GPIO_OUTCLR_PIN31_Pos;
            }

            for (int i = 0; i < 32; i++) {  // clear packet before next one arrives
                packet[i] = 0x00;
            }
