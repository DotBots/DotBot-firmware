...
static void _radio_callback_CRC(uint8_t *packet, uint8_t length) {
    ...
    memcpy(packet + length, &CRC_FAIL, 3);
    db_uart_write(DB_UART_INDEX, packet + sizeof(protocol_header_t) , length - sizeof(protocol_header_t ) +3 );
}


static void _radio_callback(uint8_t *packet, uint8_t length) {
    //memcpy(packet + length, &retour, 2); //plus utilisé avec python
    uint8_t RSSI = db_radio_rssi();
    memcpy(packet + length, &RSSI, 1);
    db_uart_write(DB_UART_INDEX, packet + sizeof(protocol_header_t) , length - sizeof(protocol_header_t) +1 );
...
int main(void)
{
    ...
    db_radio_init_CRC_Callback(&_radio_callback_CRC);
